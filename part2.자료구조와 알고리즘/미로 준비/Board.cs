...
namespace DataStructure_Algorithm
{
    ...
    {
        public TileType[,] Tile { get; private set; }// 2d좌표(인덱스)를 편하기 쓰기위해 2차원 배열을쓰자
        public int Size { get; private set; }

        public int DestY { get; private set; }
        public int DestX { get; private set; }

        Player _player; // 맵상에 움직이는 Player를 구현하기위해 새로운 클래스를 만들고 그것을 호출
        ...
        public void Initialize(int size, Player player)
        {
            ...
            // GenerateByBinaryTree(Size); // 일단 격자꼴로 empty만들고 그점들을 기준으로 우로갈지 하로갈지
             GenerateBySideWinder(Size); // 격자로 뚫고 우하이동은 같으나, 그후 다음점이 아니라 이동한위치에서 다시 우하.
        }
        ...
        public void GenerateByBinaryTree(int size)
        {
            ...
            for (int y = 0; y < Size; y++)
            {
                for (int x = 0; x < Size; x++)
                {
                    ...
                    // 외곽벽을 건드리지 않도록 해보자.
                    if (y == Size - 2) // 타일이 미로 맨아래벽 바로 위라면
                    {
                        Tile[y, x + 1] = TileType.Empty; // 무조건 우측으로 가도록 설정(아래로 가면 벽뚫리니까)
                        continue; 
                    }
                    if (x == Size - 2)
                    {
                        Tile[y + 1, x] = TileType.Empty; // 무조건 하단으로 가도록
                        continue;
                    }
                    if (rand.Next(0, 2) == 0)
                    {
                        Tile[y, x + 1] = TileType.Empty; // x+1 ?=  우측을 뚫는다
                    }
                    else // random이니 0아니면 1이 나올것
                        Tile[y+1, x] = TileType.Empty; // 하단뚫기
            ...
            for (int y = 0; y < Size; y++)
            {
                ...
                for (int x = 0; x < Size; x++)
                {
                    ...
                    else // 여기가 Binary와의 차이. 무조건 아래가 아니라 지금까지 뚫은점 중에서 하나에 하단
                    {
                        ...
                        Tile[y + 1, x - randomIndex * 2] = TileType.Empty; // 하단뚫기
                        count = 1;
                    }
        ...
        public void Render()
        {
            ...
            for (int y = 0; y < Size; y++)
            {
                for (int x = 0; x < Size; x++)
                {
                    // 플레이어 좌표를 갖고와서, 현재 좌표랑 일치하면 플레이어 전용색상으로 표시
