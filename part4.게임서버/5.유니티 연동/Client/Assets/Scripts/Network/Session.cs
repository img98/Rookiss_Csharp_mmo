...
namespace ServerCore
{
    public abstract class PacketSession : Session
    {
        ...
        {
            int processLen = 0;
            int packetCount = 0; //패킷 몇개보냈는지 보고싶어서 만듬
    ...
    public abstract class Session
    {
        ...
        RecvBuffer _recvBuffer = new RecvBuffer(65535); //받을수있는 최대 사이즈 65535 = 64kb
        ...
        public void Send(List<ArraySegment<byte>> sendBuffList) //잡큐에서 list를 send하는걸 구현하기 위해, 새로운 버전을 만듬
        {
            if (sendBuffList.Count == 0) //들어있는게 없다면
                return; // 실질적으로 아무것도 넣지않고 send를 하면 OnSendComplete에서 에러가 날거라서 바로 튕겨내질거임. 그걸 막기위해 아예 send못하게 리턴

            lock (_lock) //멀티쓰레드 개념을 생각해야하니, 보내는동안 다른 데이터로 훼손되지않게 lock을 사용하자.
            {
                foreach(ArraySegment<byte> sendBuff in sendBuffList) //_sendQueue에 넣을게 하나가 아니니(리스트니까) foreach문으로 전부 넣어주기
        ...
        void RegisterSend()
        {
            ...
            try //위에 _disconneted==1로 연결확인은 했지만, 절묘하게 저 코드 후에 바로 끊길수도있으니 일단 트라이 캐치문으로 감싸줘서 오류 확인할수있게했다.
            {
