...
namespace ServerCore
{
    public abstract class PacketSession : Session
    {
        ...
        public sealed override int OnRecv(ArraySegment<byte> buffer) // sealed = PacketSession을 상속받아 만드는애들은 OnRecv를 더이상 override할수없음(봉인) 
        {
            int processLen = 0;
            ...
            while(true)
            {
                ...
                if (buffer.Count < HeaderSize)//size 2바이트가 왔는지확인
                    break; //헤더도 안왔으면 break

                //패킷이 완전체로 도착했는지 확인
                ushort dataSize = BitConverter.ToUInt16(buffer.Array, buffer.Offset);
                if (buffer.Count < dataSize) //데이터사이즈만큼 안왔으면 잘못온거임
                    break;
            ...
            return processLen; //위 while문이 끝났다는거는, 지금까지 처리가능한 모든 데이터를 다처리했다는거고, 그게 얼마만큼의 크기였는지 return;
        }

        public abstract void OnRecvPacket(ArraySegment<byte> buffer); //OnRecv를 봉인했으니, 상속받을애들은 이걸로 만들어라, OnRecv바꾸지말고
    }
